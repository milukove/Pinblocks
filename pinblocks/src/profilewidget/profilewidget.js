...
/**
 * Register Profile Widget block
 * @since 1.0.0
 */
registerBlockType( 'pinblocks/profile-widget', {
	title: __( 'Profile Widget', 'pinblocks' ),
	icon: pinIcon,
	category: 'pinblocks',
	attributes: {
		blockId: {
	...
	edit: function( props ) {
		...
		return (
			...
			<Fragment>
				<Placeholder
					label={ __( 'Pinterest Profile Widget', 'pinblocks' ) }
				>
					<div>
						{pinIcon}
					</div>
					<TextControl
						label={ __( 'Copy Pinterest user URL here', 'pinblocks' ) }
						value={ props.attributes.href }
						onChange={
							function(e) {
								props.setAttributes( { href: e } );
